...
export function magicalSum(m: number, k: number, nums: number[]): number {
    ...
    const powtab: bigint[][] = Array.from({ length: n }, () => Array(m + 1).fill(0n));
    for (let i = 0; i < n; i++) {
        powtab[i][0] = 1n;
        for (let c = 1; c <= m; c++) {
    ...
    const comb: bigint[][] = Array.from({ length: m + 1 }, () => Array(m + 1).fill(0n));
    for (let i = 0; i <= m; i++) {
        comb[i][0] = 1n;
        for (let j = 1; j <= i; j++) {
